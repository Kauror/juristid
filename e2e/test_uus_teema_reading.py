...
**This file is skipped by default since docs/adr/0088.** The lawyers' first
feedback round withdrew the suggestion area from `Uus teema`, so the workflow
below is not one anybody meets on the shipped configuration. It is kept, rather
than deleted, because the capability is kept: it is what a reinstatement is
checked against, and the switch to run it is named beside `pytestmark`.
"""
