...
def test_the_whole_lawyer_workflow(page, base_url, screenshots):
    ...
    expect(page.get_by_role("heading", name="Teen")).to_be_visible()
    expect(page.get_by_role("heading", name="Ootan / kontrollin")).to_be_visible()
    ...
    page.locator("#id_next_kind").select_option("WAIT")
    ...
    expect(page.locator(".event--entry").filter(has_text="Kohtumine")).to_have_count(1)
    screenshots(page, "05-komposer-jarel")

    # The Matter has moved from Teen to Ootan / kontrollin.
    page.get_by_role("link", name="Minu töö").click()
    waiting = page.locator("section", has=page.get_by_role("heading", name="Ootan / kontrollin"))
    ...
    page.locator("select[name='olek']").select_option("koik")
    page.locator("select[name='ulatus']").select_option("koik")
    page.locator("select[name='hetkeseis']").select_option("")
    page.get_by_role("button", name="Filtreeri").click()
...
def test_the_composer_rejects_an_incomplete_deadline_without_losing_the_entry(page, base_url):
    ...
    expect(
        page.locator(".event--entry").filter(has_text="See tekst peab alles jääma")
    ).to_have_count(0)
