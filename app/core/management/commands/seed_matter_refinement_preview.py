"""One populated synthetic Matter for the Teema design-refinement preview.

**Preview-only. Not part of the product, not part of any release.**

`design-handoff/matter-page-refinement` is a visual study drawn from a synthetic
Matter, and the refinement is a hierarchy change: fewer top-level blocks, three
distinguishable grounds, dated obligations raised. None of that can be judged on
an empty file — a page with one entry and no deadlines looks tidy under any
arrangement. So this command builds the density the design was drawn at, through
the ordinary services, and both `/teemad/<pk>/` and the preview route render the
*same* Matter so the two screenshots differ only in presentation.

Everything here is obviously synthetic and self-evidently not Koda business
data. The record is marked `TEST` for the same reason the seeded worlds are: it
has to be impossible to mistake it for the register.

Refuses to run against a configuration that claims to hold real data, and
refuses to run outside DEBUG — the same two guards `seed_dev_data` carries.
"""
...
from app.workflow.enums import DatePrecision, Track
