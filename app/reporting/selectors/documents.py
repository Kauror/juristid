...
changed with ADR 0072 is what ``PENDING`` *predicts*: corpus-wide extraction is
...
* **read at intake** — ``INTAKE_READ``: a person watched this file being read
  ...
  ever made from it. Terminal and successful (docs/adr/0072);
...
def searchable_document_coverage(context: ReportingContext) -> MetricResult:
    ...
    if intake:
        notes.append(
            ...
            f"teadlik valik (docs/adr/0072), mitte tegemata töö."
        )
