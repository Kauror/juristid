"""`MATTER_TITLE_CHANGED`, for the full Teema edit page.

A `choices` change on an existing column, exactly like 0014: Postgres stores
the value as text either way, so no data moves, no row is rewritten, and the
append-only trigger is untouched. It is here because Django's autodetector
records the choice list in migration state, not because the database needs it.
"""

# Generated by Django 5.2.17 on 2026-08-24 14:57
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('audit', '0014_brief_summary_and_policy_area_events'),
    ]
