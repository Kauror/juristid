"""`Menetluse areng` becomes `Märge` in the words a reader sees.

**Labels only.** Every stored `event_type` value is untouched — the identifiers
stay `PROCEDURAL_DEVELOPMENT_*`, which is what historical rows hold and what
every query, test and report names. Renaming those would rewrite what the audit
trail says happened.

Django writes an `AlterField` whenever a `TextChoices` label moves, and the SQL
is a no-op: `choices` is validation metadata, not a column definition. No row is
read or rewritten.

The term was retired as a user-facing concept when `+ Märge` became the one
panel that records one. `Kõik muudatused` had gone on telling lawyers their
notes were «menetluse arengud» — a phrase the launcher has never shown them
(QA-012).
"""

# Generated by Django 5.2.17 on 2026-09-21 16:27
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("audit", "0026_timeline_steps_changed_event"),
    ]
