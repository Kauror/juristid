"""Four `Väline seisukoht` event types join the `ChangeEvent` vocabulary.
...
any of these four types exists, because nothing could write one until the
services in this release did. Reclassifying an existing `ENTRY_ADDED` row as a
recorded position would be the application inventing a professional act that
nobody performed, and `ChangeEvent` is append-only besides (docs/adr/0084 §7).
"""
