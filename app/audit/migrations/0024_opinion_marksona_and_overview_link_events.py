"""Four event types for an opinion's own `Märksõnad` and `Ülevaade / uudis` links.
...
migration is reversible to a list nothing validates against — the same shape, and
the same reasoning, as `0023_procedural_development`.

The four are `SUBMISSION_TAG_ASSIGNED`, `SUBMISSION_TAG_REMOVED`,
`SUBMISSION_OVERVIEW_LINKED` and `SUBMISSION_OVERVIEW_UNLINKED`: one per fact that
can actually move, so adding two keywords and removing one is three rows. None of
them is in `TIMELINE_EVENT_TYPES`, beside `TAG_ASSIGNED` and for the same reason —
classifying a letter is data management rather than authored chronology
(docs/adr/0093 §3).
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('audit', '0023_procedural_development'),
    ]
