"""`Menetluse link` learns two events: one recorded, one corrected.

**State-only.** One `AlterField` over a `choices` list, which in Django is
validation and display and never a database object — there is no schema change
here, no index, no constraint and no data touched.

Neither value is in `matters.timeline.TIMELINE_EVENT_TYPES`, exactly as the four
`Ülevaade / uudis` events and the four `Väline seisukoht` events are not. The
chronology renders structured facts from their canonical records, and a
procedural link is not a chronology fact at all: it is *where the file is
happening*, not something that happened to it, so a row saying «EIS link added»
on a Tuesday would be the history of somebody's typing (docs/adr/0089 §11).
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('audit', '0021_overview_news_event_labels'),
    ]
