"""`SUBMISSION_CORRECTED` on `ChangeEvent.event_type`'s choices.

`event_type` is a `CharField` with `choices`, so a new value is a state change
Django records with no SQL behind it — `sqlmigrate` prints a no-op.

The value is new, not renamed: before QA-023 a recorded send had no correction
surface at all, so nothing historical says `SUBMISSION_CORRECTED`. It is its own
event rather than a second `SUBMISSION_SENT`, which would make the file read as
two letters, and rather than a `SUBMISSION_WITHDRAWN`, which says something
about the letter where this says something about what we wrote down about it.
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("audit", "0028_removal_event_types"),
    ]
        ...
        migrations.AlterField(
            ...
            field=models.CharField(
                choices=[
                    ...
                    ("SUBMISSION_CORRECTED", "Arvamuse andmeid parandatud"),
                    ...
                    ("TIMELINE_STEPS_CHANGED", "Menetluse kulgu muudetud"),
                    ("PROCEDURAL_LINK_RECORDED", "Menetluse link lisatud"),
                    ("PROCEDURAL_LINK_CORRECTED", "Menetluse linki parandatud"),
                    ("PROCEDURAL_DEVELOPMENT_RECORDED", "Märge lisatud"),
                    ("PROCEDURAL_DEVELOPMENT_CORRECTED", "Märget parandatud"),
                    ("PROCEDURAL_DEVELOPMENT_DOCUMENT_LINKED", "Märke fail lisatud"),
                    ("ENTRY_REMOVED", "Sissekanne eemaldatud"),
                    ("PROCEDURAL_DEVELOPMENT_REMOVED", "Märge eemaldatud"),
                    ("ENGAGEMENT_REMOVED", "Kaasamine eemaldatud"),
                    ("EXTERNAL_POSITION_REMOVED", "Väline seisukoht eemaldatud"),
                    ("WEBSITE_OVERVIEW_REMOVED", "Ülevaade / uudis eemaldatud"),
                    ("IMPORTANT_DATE_REMOVED", "Oluline tähtaeg eemaldatud"),
                    ("EFFECTIVE_DATE_REMOVED", "Jõustumine eemaldatud"),
                    ("WORK_VICTORY_REMOVED", "Töövõidu kirje eemaldatud"),
                ],
                db_index=True,
                max_length=64,
            ),
