"""The eight `_REMOVED` event types, on `ChangeEvent.event_type`'s choices.

`event_type` is a `CharField` with `choices`, so a new value is a state change
Django records even though the column, its length and its index are all
untouched — there is no SQL beyond what `sqlmigrate` prints as a no-op.

The values are new, not renamed: nothing historical says `ENTRY_REMOVED`,
because before OWNER-04 nothing could be removed. One per record family rather
than a shared `RECORD_REMOVED`, so `Kõik muudatused` goes on saying which
record left the file (docs/adr/0102).
"""
