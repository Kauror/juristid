...
| `engagement-deadline-before-round`, `submission-sent-in-future`, `closed-matter-open-next-action`, `closed-matter-planned-website-overview`, `closed-matter-open-feedback-wait` | a row breaks a rule the services now refuse to write; no constraint guards it | These do **not** block the migration. Record the identifiers for review and carry on. Imported archive rows are reported, never rewritten. |
