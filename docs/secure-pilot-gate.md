...
| 6 | Upload type/size controls plus a malware scanning and quarantine path | technical owner | in the repository — allow-list and size limit enforced; the scanner is wired (ADR 0066). ClamAV runs as an internal, pinned, healthchecked container on both deployment stacks and is the only thing that writes `CLEAN`; an unreachable scanner leaves a file `PENDING` and no parser opens it. CI proves real detection against the container with the EICAR test file, and real data with no scanner configured refuses to start (`juristid.E015`). That the scanner is actually *running and current* on a given host is **deployment-time verification required** |
