...
- secret management;
- upload type/size controls and malware scanning/quarantine path (scanning removed by ADR 0072; the upload controls are unchanged and are now the whole of this line that is implemented);
...
- malware/validation state (the `malware_scan_state` column survives as dead schema; nothing writes or reads it — ADR 0072);
...
- malware scan/quarantine before general use where real data is involved — **not implemented; withdrawn by ADR 0072 as a product decision, and recorded as unmet in `docs/secure-pilot-gate.md` row 6 rather than reworded away**;
